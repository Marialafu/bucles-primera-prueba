...
const calculateAverage = (numbers) => {
  ...
  for (const number of numbers){
    calculatedAverage = calculatedAverage + number
  }
...
const divideNumberIntoPositiveAndNegative = (numbers) => {
  ...
  console.log(
    `Positivos: ${positiveNumbers}. Negativos: ${negativeNumbers}`
  )
}
...
const eliminateWordsThatStartWithVowels = (words) => {
  ...
  for (const word of words){
    if (vowels.includes(word.charAt(0).toUpperCase())){wordsStartWithConsonantArray.push(word)}
  }
...
const clarifyPropertyOfNumber = (numbers) => {
  ...
  for(const number of numbers) {
    if (number % 2 === 0 && number !== 0){
      pairNumbers = pairNumbers + number
    } else if (number % 2 !== 0){
      oddNumbers = oddNumbers + number
    } else if (number === 0){
      amountOf0 = amountOf0 + number
    }
  }
  console.log(`Pares: ${pairNumbers.length} - Impares: ${oddNumbers.length} - Ceros: ${amountOf0.length}`);
}
...
identificateMoreThan5LettersPosition(['Pedro', 'Ana', 'Esteban', 'María', 'Begoña'])
...
securityCode(['Fuego en el bosque', 'Refugio seguro', 'Suministros esenciales'])
...
const equalNumbersList = (firstList, secondList) =>{
  const equalNumbers = []
  for (const firstListnumber of firstList){
    for (const secondListNumber of secondList){
      if (firstListnumber === secondListNumber){
        equalNumbers.push(firstListnumber)
      }
    }
  }
...
const identifyNumberOfVowels = (words) => {
  ...
  for(const word of words){
    if (vowels.includes(word)){
      totalVowelsForEachWord.push(word)
    }
  }
...
const reverseEachWord = (words) => {
  ...
  const separateReverseWord = reverseWord.split('H')
  console.log(separateReverseWord);
